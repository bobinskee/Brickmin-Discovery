...
func _update_min(brickmin: CharacterBody3D, delta: float, dict_individual: Dictionary, _dict_all: Dictionary):
	...
	var player_cursor = brickmin.leader.get_parent().get_parent().get_child(1).get_child(0)
	...
	if brickmin.velocity.is_finite():
		...
		if brickmin.leader:
			...
			elif brickmin.leader.swarming:
				target_position = player_cursor.global_position
				start_distance = 10
				stop_distance = 0
			
			else:
				target_position = brickmin.leader.global_position
				start_distance = 8
				stop_distance = 8
			...
			if not brickmin.made_it:
				...
				if not brickmin.leader.swarming:
					
					if brickmin.leader.velocity.length() > 0.0:
						start_distance = 8
						stop_distance = 6
					
					if abs(brickmin.global_position.distance_to(target_position)) >= (stop_distance) or abs(brickmin.leader.velocity.length()) > 0.0:
					...
					if xz_dist >= (stop_distance) or abs(brickmin.leader.velocity.length()) > 0.0:
						brickmin.following = true 
			...
			if brickmin.following:
				...
				if not brickmin.leader.swarming:
					...
					if (abs(xz_dist - brickmin.xz_rand) <= brickmin.fallback) and abs(brickmin.leader.velocity.length()) > 0.0:
			...
			for leader in (BrickminManager.leader_bodies):
				var distance = brickmin.global_position.distance_to(leader.global_position + (Vector3.ZERO * 1.5))
				
				if distance < brickmin.space_leader:
					repel_force += ((brickmin.global_position - leader.global_position).normalized()/distance) * (adjust_speed * 2)
			...
			if dict_individual["near_cliff"]:
				...
				if brickmin.leader.is_on_floor():
					brickmin.wanna_jump = true
					
					if dict_individual["jump_to"]: 
						if (dict_individual["jump_to"] - brickmin.global_position).dot(brickmin.leader.global_position - brickmin.global_position) > 0.5:
					...
					if abs(brickmin.global_position.distance_to(brickmin.leader.global_position)) < 5:
						cur_move = Vector3.ZERO
